function [S_skel,S_radii,length_node_to_remove_par] = Skeketon_clean_up_3(S_skel, S_radii,CropSize,Judging_ratio)
...
S_link_g_r_accum = accumarray(S_link.group,S_link.radii);
S_link_g_l_accum = accumarray(S_link.group,1);
S_link_g_l_accum = S_link_g_l_accum.*S_link_g_l_accum./S_link_g_r_accum;


S_link_group.ratio = S_link_g_l_accum(S_link_group.name);



%%%%%%%%%%%%%%%%%%%%%%%
badLinks = S_link_group.name(S_link_group.ratio<Judging_ratio);
%%%%%%%%%%%%%%%%%%%%%%%
...
for ii = 1:length(wierdNode_group.name)
    wierdNode_out_name_par{ii} = unique(wierdNode_out_name_par{ii});
